...
class Enigma(models.Model):
    ...
    # IL VECCHIO CAMPO 'risposta_corretta' VA ELIMINATO DA QUI
...
# NUOVO: Modello per gli allegati
...
# NUOVO: Modello per i campi risposta
class CampoRisposta(models.Model):
    ...
    # --- RIGA AGGIUNTA ---
    is_domanda_principale = models.BooleanField(
        default=False,
        help_text="Spunta questa casella se questa è la domanda principale che assegna il punteggio base."
    )
...
# Modello RispostaUtente aggiornato
class RispostaUtente(models.Model):
    # AGGIUNTA CHIAVE: 'related_name' risolve l'errore nella classifica
    ...
    # SOSTITUITO 'is_corretta' con questo campo
...
# NUOVO: Modello per le singole risposte multiple
...
# I tuoi modelli esistenti (Profile, Suggerimento, Badge, ecc.) rimangono qui sotto.
# Li lascio invariati perché funzionano con il resto del codice.
...
class Suggerimento(models.Model):
    enigma = models.ForeignKey(Enigma, related_name='suggerimenti', on_delete=models.CASCADE)
    ...
    class Meta:
        ...
        ordering = ['ordine']
    def __str__(self):
        return f"Suggerimento {self.ordine} per {self.enigma.titolo}"

class Badge(models.Model):
    nome = models.CharField(max_length=100)
    descrizione = models.TextField()
    icona = models.CharField(max_length=50, help_text="Es. 'bi bi-star'")
    def __str__(self):
        return self.nome
